...
class Arcane_TeamWars_Widget extends WP_Widget {
    ...
    {
        $widget_ops = array('classname' => 'widget_other_matches', 'description' => esc_html__('TeamWars widget', 'arcane'));
        parent::__construct('teamwars', esc_html__('SW TeamWars', 'arcane'), $widget_ops);

        $this->default_settings = array('title' => esc_html__('TeamWars', 'arcane'),
                'show_limit' => 10,
                'hide_title' => false,
                'hide_older_than' => '1m',
                'visible_games' => array());
    ...
    function widget($args, $instance) {
        ...
        foreach($_games as $g) {
            ...
            $m = $ArcaneWpTeamWars->get_match(array('status' => array('active', 'done'),'from_date' => $from_date, 'game_id' => $g->id, 'limit' => $instance['show_limit'], 'order' => 'desc', 'orderby' => 'date', 'sum_tickets' => true), false, true);
...
<div class="teamwar-list">
    ...
    <ul class="matches-tab">
        ...
        ?>
            <li <?php if($i == 0) echo 'class="active"'; ?>>
                <a class="custom-tabs-link" href="#<?php echo esc_attr($link); ?>" title="<?php echo esc_attr($game->title); ?>"><?php echo esc_attr($game->abbr); ?></a>
            </li>
        <?php } endfor; ?>
        </ul>
    ...
    <?php
        ...
        if (isset($matchesnew) AND is_array($matchesnew)) {
    ...
    <?php foreach($matches as $i => $match) :
            ...
            if ($type == 'user') {
               $pf_url =  get_user_meta($team1id, 'profile_photo',true);
                ...
                $image1 = arcane_aq_resize( $img_url1, 25, 25, true, true, true ); //resize & crop img

                $team2id = $match->team2;
                $img_url2 = get_post_meta($team2id, 'team_photo', true);
                $image2 = arcane_aq_resize( $img_url2, 25, 25, true, true, true ); //resize & crop img
            }
    ...
    <li data-type="#<?php echo esc_attr( $match->game_id ); ?>" class="teamwar-item">
            ...
            <div class="wrap">
                <?php echo '<a href="' . esc_url(get_permalink($match->ID)) . '" ddata-tooltip="' . esc_attr($match->title) . '">'; ?>
                <?php if($is_upcoming) : ?>
                ...
                <?php if(!isset($image1) or empty($image1)){ $image1 = get_theme_file_uri('img/defaults/team25x25.jpg');  } ?>
                <?php if(!isset($image2) or empty($image2)){ $image2 = get_theme_file_uri('img/defaults/team25x25.jpg');  } ?>
                <div class="match-wrap">

                    <img src="<?php echo esc_url($image1);?>" class="team1img" alt="<?php echo esc_attr($match->title); ?>" />

                    <span class="vs"><?php esc_html_e("vs","arcane");?></span>
                    <div class="opponent-team">
                    <img src="<?php echo esc_url($image2);?>" class="team1img" alt="<?php echo esc_attr($match->title); ?>" />
                    </div>
                </div>
                <div class="date"><?php echo esc_html($gameabr); ?> - <?php echo esc_html($date); ?></div>
                </a>
            </div>
    ...
    ?>
        <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'arcane'); ?></label> <input class="widefat" name="<?php echo esc_attr($this->get_field_name('title')); ?>" id="<?php echo esc_attr($this->get_field_id('title')); ?>" value="<?php echo esc_attr($title); ?>" type="text" /></p>

        <p>
            <input class="checkbox" name="<?php echo esc_attr($this->get_field_name('hide_title')); ?>" id="<?php echo esc_attr($this->get_field_id('hide_title')); ?>" value="1" type="checkbox" <?php checked($instance['hide_title'], true)?>/> <label for="<?php echo esc_attr($this->get_field_id('hide_title')); ?>"><?php esc_html_e('Hide title', 'arcane'); ?></label>
        </p>
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('show_limit')); ?>"><?php esc_html_e('Show matches:', 'arcane'); ?></label> <input class="cetpet" name="<?php echo esc_attr($this->get_field_name('show_limit')); ?>" id="<?php echo esc_attr($this->get_field_id('show_limit')); ?>" value="<?php echo esc_attr($show_limit); ?>" type="text" /></p>
        <p><label for="<?php echo esc_attr($this->get_field_id('hide_older_than')); ?>"><?php esc_html_e('Hide older than', 'arcane'); ?></label><br/><select name="<?php echo esc_attr($this->get_field_name('hide_older_than')); ?>" id="<?php echo esc_attr($this->get_field_id('hide_older_than')); ?>">
...
function arcane_return_team_wars_widget(){
	register_widget("Arcane_TeamWars_Widget");
}
// register widget
add_action('widgets_init', 'arcane_return_team_wars_widget');
?>
            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        //$import_message = esc_html__( 'Import Demo', 'arcane' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        //$import_message = esc_html__( 'Demo Imported', 'arcane' );
                    }

					if(esc_html( apply_filters( 'wbc_importer_directory_title', $imports['directory'] ) ) == 'Arcane'){
						$view_demo = '<a id="demo_link" target="_blank" href="https://skywarriorthemes.com/arcane/">'.esc_html__( 'View Demo', 'arcane' ).'</a>';
					}
                    ...
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'arcane' ).'</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', 'arcane' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', 'arcane' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'arcane' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'arcane' ).'</div>';
                    }
                    echo '</div>';
                    echo '</div>';
                ...
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'arcane' )."</h5>";
            }

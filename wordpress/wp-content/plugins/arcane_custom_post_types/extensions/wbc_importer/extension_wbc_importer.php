...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
            ...
            if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
                ...
                if ( file_exists( $demo_data_loc.'/'.$import_parts['content_file'] ) && is_file( $demo_data_loc.'/'.$import_parts['content_file'] ) ) {
                    ...
                    if ( !isset( $import_parts['imported'] ) || true === $reimporting ) {
                        ...
                        echo esc_html__( "Demo Already Imported", 'arcane' );
                    }
                }
        ...
        function add_importer_section() {
            ...
            $wbc_importer_label = trim( esc_html( apply_filters( 'wbc_importer_label', __( 'Demo Importer', 'arcane' ) ) ) );

            $wbc_importer_label = ( !empty( $wbc_importer_label ) ) ? $wbc_importer_label : __( 'Demo Importer', 'arcane' );

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">'. apply_filters( 'wbc_importer_description', esc_html__( '***PLEASE INSTALL AND ACTIVATE ALL SUGGESTED PLUGINS BEFORE DEMO IMPORT TO GET BEST RESULT***', 'arcane')).'<br/>'.apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress. If you\'re not using a fresh install, make sure you backup your current theme options as the importer will override them. Import will take some time to finish. If you see error 500 in the console, import failed
				because of the low server resources. You can refresh the page and try again until it goes through, or you can add set_time_limit(600); to your wp-config.php file. Please remove WordPress Importer plugin because of conflict.', 'arcane' ) ).'</p>',

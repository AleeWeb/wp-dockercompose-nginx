...
class CMB2_Field {
	...
	public function render_field() {
		...
		if ( 'title' == $this->type() || ! $this->args( 'show_names' ) ) {
			...
			if ( ! $this->args( 'show_names' ) ) {
				$stil = 'title' == $this->type() ? '' : ' ';
				printf( "\n<label%s for=\"%s\">%s</label>\n", $stil, $this->id(), $this->args( 'name' ) );
			}
		} else {
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'arcane' ),
			'remove_button' => esc_html__( 'Remove Group', 'arcane' ),
		) ) : $args['options'];
		...
		if ( in_array( $args['type'], $option_types, true ) ) {
			...
			$args['show_option_none'] = true === $args['show_option_none'] ? esc_html__( 'None', 'arcane' ) : $args['show_option_none'];

			if ( ! $args['show_option_none'] ) {
				$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
				$args['show_option_none'] = $off_by_default ? false : esc_html__( 'None', 'arcane' );
			}

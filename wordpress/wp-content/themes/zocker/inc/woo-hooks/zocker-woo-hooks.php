...
add_filter( 'loop_shop_per_page', 'zocker_new_loop_shop_per_page', 20 );
function zocker_new_loop_shop_per_page( $product_count ) {
  $product_count = zocker_opt( 'zocker_woo_product_perpage' );
...
add_filter('woocommerce_product_loop_start','zocker_woocommerce_product_loop_start');
function zocker_woocommerce_product_loop_start( ) {
    echo '<div class="row">';
}

// wooocommerce product loop start filter
add_filter('woocommerce_product_loop_end','zocker_woocommerce_product_loop_end');
function zocker_woocommerce_product_loop_end( ) {
    echo '</div>';
}

// woocommerce format sale price
add_filter('woocommerce_format_sale_price','zocker_woocommerce_format_sale_price',10,3);
function zocker_woocommerce_format_sale_price( $price, $regular_price, $sale_price ) {
...
add_filter('woocommerce_review_gravatar_size','zocker_woocommerce_review_gravatar_size');
function zocker_woocommerce_review_gravatar_size() {
    return 65;
}
...
add_filter('woocommerce_output_related_products_args','zocker_woocommerce_output_related_products_args',10,1);
function zocker_woocommerce_output_related_products_args( $args ) {
    if( class_exists('ReduxFramework') ) {
        $args['posts_per_page'] = zocker_opt('zocker_woo_relproduct_num');
    } else {
...
add_filter('woocommerce_upsell_display_args','zocker_woocommerce_upsell_display_args',10,1);
function zocker_woocommerce_upsell_display_args( $args ) {
    if( class_exists('ReduxFramework') ) {
        $args['posts_per_page'] = zocker_opt('zocker_woo_upsellproduct_num');
    } else {
        $args['posts_per_page'] = '3';
    }
    return $args;
...
function woocommerce_cross_sells_total( $limit ) {
    if( class_exists('ReduxFramework') ) {
        $limit = zocker_opt('zocker_woo_crosssellproduct_num');
    } else {
        $limit = '3';
    }
...
// Ajax Cart Count
add_filter( 'woocommerce_add_to_cart_fragments', 'zocker_refresh_mini_cart_count');
function zocker_refresh_mini_cart_count($fragments){
    ob_start();
    ?>
...
// Checkout fields
add_filter( 'woocommerce_checkout_fields' , 'zocker_override_checkout_fields' );
function zocker_override_checkout_fields( $fields  ){
    ...
    $fields['billing']['billing_first_name']['placeholder'] = esc_attr__('First Name', 'zocker');
    ...
    $fields['billing']['billing_email']['placeholder'] = esc_attr__('Email address', 'zocker');
    ...
    $fields['billing']['billing_city']['placeholder'] = esc_attr__('Town / City', 'zocker');
    ...
    $fields['billing']['billing_postcode']['placeholder'] = esc_attr__('Postcode', 'zocker');
    ...
    $fields['shipping']['shipping_first_name']['placeholder'] = esc_attr__('First Name', 'zocker');
    ...
    $fields['shipping']['shipping_last_name']['placeholder'] = esc_attr__('Last Name', 'zocker');
    ...
    $fields['shipping']['shipping_company']['placeholder'] = esc_attr__('Company name (optional)', 'zocker');
    ...
    $fields['shipping']['shipping_phone']['placeholder'] = esc_attr__('Phone', 'zocker');
    ...
    $fields['shipping']['shipping_email']['placeholder'] = esc_attr__('Email address', 'zocker');
    ...
    $fields['shipping']['shipping_city']['placeholder'] = esc_attr__('Town / City', 'zocker');
    ...
    $fields['shipping']['shipping_postcode']['placeholder'] = esc_attr__('Postcode', 'zocker');
...
add_filter( 'woocommerce_default_address_fields', 'zocker_override_default_locale_fields' );
function zocker_override_default_locale_fields( $fields ) {
...
add_filter( 'woocommerce_billing_fields' , 'zocker_custom_billing_fields' );
function zocker_custom_billing_fields( $fields ) {
    ...
    $fields['billing_first_name']['placeholder'] = esc_attr__('First Name', 'zocker');
    ...
    $fields['billing_last_name']['placeholder'] = esc_attr__('Last Name', 'zocker');
    ...
    $fields['billing_company']['placeholder'] = esc_attr__('Company name (optional)', 'zocker');
    ...
    $fields['billing_phone']['placeholder'] = esc_attr__('Phone', 'zocker');
    ...
    $fields['billing_email']['placeholder'] = esc_attr__('Email address', 'zocker');
    ...
    $fields['billing_city']['placeholder'] = esc_attr__('Town / City', 'zocker');
    ...
    $fields['billing_postcode']['placeholder'] = esc_attr__('Postcode', 'zocker');
...
add_filter( 'woocommerce_shipping_fields' , 'zocker_custom_shipping_fields' );
function zocker_custom_shipping_fields( $fields ) {

    // Shipping Fields

    $fields['shipping_first_name']['class'] = array('col-lg-6');
    $fields['shipping_first_name']['input_class'] = array('form-control');
    $fields['shipping_first_name']['placeholder'] = esc_attr__('First Name', 'zocker');
    ...
    $fields['shipping_last_name']['placeholder'] = esc_attr__('Last Name', 'zocker');
    ...
    $fields['shipping_company']['placeholder'] = esc_attr__('Company name (optional)', 'zocker');
    ...
    $fields['shipping_phone']['placeholder'] = esc_attr__('Phone', 'zocker');
    ...
    $fields['shipping_email']['placeholder'] = esc_attr__('Email address', 'zocker');
    ...
    $fields['shipping_city']['placeholder'] = esc_attr__('Town / City', 'zocker');
    ...
    $fields['shipping_postcode']['placeholder'] = esc_attr__('Postcode', 'zocker');
...
add_action('woocommerce_before_main_content','zocker_shop_col_start_cb');

// Shop column end wrapper hook
add_action('woocommerce_after_main_content','zocker_shop_col_end_cb',10);

// shop main content hook
add_action('zocker_shop_main_content','zocker_shop_main_content_cb',10);

// shop main content end hook
add_action('zocker_shop_main_content_end','zocker_shop_main_content_end_cb',10);

// zocker woocommerce pagination hook
add_action('woocommerce_after_shop_loop','zocker_woocommerce_pagination',10);

// zocker woocommerce filter wrapper hook
add_action('woocommerce_before_shop_loop','zocker_woocommerce_filter_wrapper',20);

// zocker woocommerce product content wrapper
add_action('zocker_woocommerce_product_content', 'zocker_woocommerce_tab_content_wrapper_start', 10 );
add_action('zocker_woocommerce_product_content', 'zocker_grid_tab_content_cb', 10 );
add_action('zocker_woocommerce_product_content', 'zocker_list_tab_content_cb', 20 );
add_action('zocker_woocommerce_product_content', 'zocker_woocommerce_tab_content_wrapper_end', 30 );

// zocker woocommerce get sidebar
add_action('woocommerce_sidebar','zocker_woocommerce_get_sidebar',10);

/*
*   Content Product Hook
*/

// zocker shop loop product thumbnail hook
add_action( 'woocommerce_before_shop_loop_item', 'zocker_loop_product_thumbnail', 10 );

// zocker shop loop hozrizontal product thumbnail hook
add_action( 'woocommerce_before_shop_horizontal_loop_item', 'zocker_loop_product_thumbnail', 10 );

// zocker shop loop product summary hook
add_action( 'woocommerce_after_shop_loop_item', 'zocker_loop_product_summary', 10 );

// zocker shop loop horizontal product summary hook
add_action('woocommerce_after_shop_horizonal_loop_item', 'zocker_horizontal_loop_product_summary' );

// zocker shop loop horizontal product summary hook
add_action('zocker_woocommerce_shop_three_loop_product_summary','zocker_woocommerce_shop_three_loop_product_summary_cb',10);

/*
*   Single Product Hook
*/
// single product before summary
add_action('woocommerce_before_single_product_summary', 'zocker_woocommerce_before_single_product_summary',10);

// single product title
add_action('woocommerce_single_product_summary', 'zocker_woocommerce_single_product_title', 20 );

// single product price and rating
add_action('woocommerce_single_product_summary', 'zocker_woocommerce_single_product_price_rating', 30 );

// single product excerpt
add_action('woocommerce_single_product_summary', 'zocker_woocommerce_single_product_excerpt', 40 );

// single product availability
add_action( 'woocommerce_single_product_summary', 'zocker_woocommerce_single_product_availability', 50 );

// single product add to cart button
add_action('woocommerce_single_product_summary','zocker_woocommerce_single_add_to_cart_button',60);

// single product meta hook
add_action('woocommerce_single_product_summary','zocker_woocommerce_single_meta',80);

// single product sidebar
add_action('zocker_woocommerce_single_product_sidebar','zocker_woocommerce_single_product_sidebar_cb');

/*
* Single product quick view
*/
add_action('zocker_quickview_before_single_product_summary','zocker_woocommerce_before_single_product_summary',10);

// single product price and rating
add_action('zocker_quickview_single_product_summary','zocker_woocommerce_single_product_price_rating',20);

// single product title
add_action('zocker_quickview_single_product_summary','zocker_woocommerce_quickview_single_product_title',30);

// single product excerpt
add_action('zocker_quickview_single_product_summary','zocker_woocommerce_single_product_excerpt',40);

// single product add to cart button
add_action('zocker_quickview_single_product_summary','zocker_woocommerce_single_add_to_cart_button',60);
...
add_action('woocommerce_review_before_comment_meta','zocker_woocommerce_reviewer_meta',10,1);
...
add_action('woocommerce_proceed_to_checkout','zocker_woocommerce_button_proceed_to_checkout',20);

// cross sell products
add_action('woocommerce_after_cart','zocker_woocommerce_cross_sell_display',10);

// mini cart view cart button
add_action('woocommerce_widget_shopping_cart_buttons','zocker_minicart_view_cart_button',10);

// mini cart checkout button
add_action('woocommerce_widget_shopping_cart_buttons','zocker_minicart_checkout_button',20);

/*
*   checkout hook
*/

// zocker woocommerce checkout login form
add_action('woocommerce_before_checkout_form','zocker_woocommerce_before_checkout_form',10);
...
add_filter( 'woosc_button_html', 'zocker_woosc_button_html', 10, 2 );
function zocker_woosc_button_html( $output , $prodid ) {
    return $output = '<a href="#" class="icon-btn3 woosc-btn woosc-btn-' . esc_attr( $prodid ) . ' ' . get_option( '_woosc_button_class' ) . '" data-id="' . esc_attr( $prodid ) . '"><i class="fal fa-layer-group"></i></a>';
}

// Compare Button Text Added
add_filter( 'woosc_button_text_added', 'zocker_woo_compare_btn_text_added', 10, 2 );
function zocker_woo_compare_btn_text_added( ){
    return $output = '<i class="fal fa-layer-group"></i>';
}

add_filter( 'woosc_button_text', 'zocker_compare_btn_text', 10, 2 );
function zocker_compare_btn_text(){
    return $output = '<i class="fal fa-layer-group"></i>';
}
...
add_filter( 'woosq_button_html', 'zocker_woosq_button_html', 10, 2 );
function zocker_woosq_button_html( $output , $prodid ) {
    return $output = '<a href="#" class="icon-btn3 woosq-btn woosq-btn-' . esc_attr( $prodid ) . ' ' . get_option( 'woosq_button_class' ) . '" data-id="' . esc_attr( $prodid ) . '" data-effect="mfp-3d-unfold"><i class="far fa-search"></i></a>';
}
...
// wishlist page title remove
add_filter('yith_wcwl_wishlist_title','__return_false');

// wishlist share remove
add_filter('yith_wcwl_share_conditions','__return_false');

// wishlist button pririty change
add_filter( 'yith_wcwl_positions', 'zocker_remove_wishlist_from_single_page' );

function zocker_remove_wishlist_from_single_page( $so_array=array() ){
    $so_array   =   array(
        "shortcode" => array(
            'hook'      => '',
            'priority'  => 5,

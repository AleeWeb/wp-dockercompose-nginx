...
if ( ! function_exists( 'zocker_breadcrumbs' ) ) {
    function zocker_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'zocker' )
        );
        $args    = apply_filters( 'zocker_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html = '';
        $html .= '<ul class="breadcumb-menu-style1 mx-auto" '.implode( ' ',  $args_el ).'>';
        // Add Homepage link (always present)
        $html .= '<li><a href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '"><i class="fal fa-home text-theme"></i>' . esc_attr( $args['home_title'] ) . '</a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<li class="active bread-search">'.esc_html__('Search results for: ','zocker') . get_search_query() . '</li>';
        } elseif ( is_404() ) {
            $html .= '<li>' . esc_html__( 'Error 404', 'zocker' ) . '</li>';
        } elseif ( is_home() ) {
        ...
        if( class_exists('woocommerce') ) {
            ...
            echo apply_filters( 'zocker_breadcrumbs_filter', $html );
        }

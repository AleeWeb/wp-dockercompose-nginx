...
class Arcane_Envato_API {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'arcane' ), '1.0.0' );
	}
	...
	public function request( $url, $args = array() ) {
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', __( 'An API token is required.', 'arcane' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, __( 'An unknown API error occurred.', 'arcane' ) );
		}
		$return = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( null === $return ) {
			return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'arcane' ) );
		}
